...
export class UsersService {
  ...
  async createUser(dto: CreateUserDto, actor: any, ip?: string) {
    const dupUsername = await this.prisma.auth_users.findFirst({
      where: { username: dto.username, deleted_at: null },
    });
    if (dupUsername) throw new ConflictException(`Username "${dto.username}" already exists`);
    const dupEmail = await this.prisma.auth_users.findFirst({
      where: { email: dto.email, deleted_at: null },
    });
    if (dupEmail) throw new ConflictException(`Email "${dto.email}" already exists`);

    const password_hash = bcrypt.hashSync(dto.password, 10);

    const created = await this.prisma.auth_users.create({
      data: {
  ...
  async updateUser(id: number, dto: UpdateUserDto, actor: any, ip?: string) {
    ...
    // Replace the additional-role set when explicitly provided
    if (dto.role_ids !== undefined) {
      const primaryId = dto.primary_role_id ?? existing.primaryRole?.id;
      await this.prisma.auth_user_role_assignments.deleteMany({
        where: { user_id: id },
      });
      const targets = dto.role_ids.filter((rid) => rid !== primaryId);
      for (const rid of targets) {
        await this.prisma.auth_user_role_assignments.create({
          data: {
            user_id: id,
            role_id: rid,
            assigned_by: actor?.userId ?? undefined,
